...
public:
    ...
    constexpr Score negamax(uint8_t depth, const BitBoard& board, Score alpha = s_minScore, Score beta = s_maxScore, bool cutNode = false)
    {
            ...
            if (isRoot && m_isPrimary) {
                ...
                if (wdl != syzygy::WdlResultFailed) {
                    ...
                    if (wdlTtFlag == core::TtExact
                        ...
                        core::TranspositionTable::writeEntry(m_stackItr->board.hash, wdlScore, s_noScore, movegen::nullMove(), depth, m_ply, wdlTtFlag);
                        return wdlScore;
                    }
        ...
        core::TranspositionTable::writeEntry(m_stackItr->board.hash, bestScore, m_stackItr->eval, bestMove, depth, m_ply, ttFlag);
        return bestScore;
    }
...
private:
    ...
    constexpr Score quiesence(const BitBoard& board, Score alpha, Score beta)
    {
        ...
        core::TranspositionTable::writeEntry(m_stackItr->board.hash, bestScore, m_stackItr->eval, bestMove, 0, m_ply, ttFlag);
        return bestScore;
    }
    ...
    bool makeMove(const BitBoard& board, movegen::Move move)
    {
        ...
        m_repetition.add(m_stackItr->board.hash);
    ...
    inline Score fetchOrStoreEval(const BitBoard& board, std::optional<core::TtEntryData> entry)
    {
        if (entry.has_value() && entry->eval != s_noScore) {
            ...
            core::TranspositionTable::writeEntry(m_stackItr->board.hash, s_noScore, eval, movegen::nullMove(), 0, m_ply, core::TtAlpha);
            return eval;
        }
    }
    ...
    {
        const bool isDraw = board.halfMoves >= 100 || m_repetition.isRepetition(m_stackItr->board.hash) || board.hasInsufficientMaterial();
        if (isDraw) {
