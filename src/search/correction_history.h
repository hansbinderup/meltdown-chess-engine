...
public:
    ...
    inline Score getCorrection(const BitBoard& board) const
    {
        ...
        const Score materialCorrection = getEntry<player>(m_materialTable, core::generateMaterialHash(board));
    ...
    inline void update(const BitBoard& board, uint8_t depth, Score score, Score eval)
    {
        ...
        updateEntry<player>(m_materialTable, core::generateMaterialHash(board), score, eval, depth);
...
private:
    /* cache settings */
    constexpr static inline size_t s_cacheKeySize { 15 };
    constexpr static inline size_t s_cacheSize { 1 << s_cacheKeySize };
    constexpr static inline uint16_t s_cacheMask { s_cacheSize - 1 };
