...
public:
    ...
    template<Player player> constexpr std::optional<movegen::Move> pickNextMove(const BitBoard& board)
    {
        ...
        case TtMove: {
            if (const auto pickedMove = pickTtMove(board))
                return pickedMove;
...
private:
    ...
    constexpr std::optional<movegen::Move> pickTtMove(const BitBoard& board)
    {
        if (!m_calledPickTtMove && m_ttMove.has_value()) {
            m_calledPickTtMove = true;
            return *m_ttMove;
        }
        m_calledPickTtMove = true;
        ...
        // TT move has already been picked
