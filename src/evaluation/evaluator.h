...
public:
    ...
    void setSearchParams(const BitBoard& board)
    {
        m_board = board;
        m_hash = engine::generateHashKey(board);
    }

    // Args: update once for each iterative deepening
    void
    startSearch(uint8_t depth, int32_t alpha, int32_t beta)
    {
    ...
    // TODO shouldn't terminate the thread loop
    // void stop()
    //{
    //    m_isStopped = true;
    //}
...
private:
    ...
    // Promise-type container for result of negamax
    std::promise<int32_t> m_searchPromise;
    std::future<int32_t> m_futureScore;

    // Updated once for entire iterative deepening: acceptable overhead
    BitBoard m_board;
    uint64_t m_hash;

    // Updated during each iterative deepening
...
private:
    ...
    constexpr movegen::Move scanForBestMove(uint8_t depth, const BitBoard& board)
    {
        ...
        for (auto& searcher : m_searchers) {
            searcher.setSearchParams(board);
        }
        ...
        while (d <= depth) {
            ...
            // TODO placeholder for thread voting
            winningThreadId = 0;
            // TODO use promise to wait on results
            std::vector<int32_t> threadScores {};
            for (auto& searcher : m_searchers) {
                threadScores.push_back(searcher.getBestScore());
            }
            int32_t score = threadScores.at(0);
        ...
        return m_searchers.at(winningThreadId).getBestPvMove();
    }

    constexpr void printScoreInfo(const BitBoard& board, int32_t score, uint8_t currentDepth)
    {
        // Placeholder: print per-thread for now
        for (auto& searcher : m_searchers) {
            searcher.printScoreInfo(board, score, currentDepth, m_startTime);
        }
    }
