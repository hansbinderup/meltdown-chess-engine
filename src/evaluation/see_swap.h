...
private:
    constexpr static inline uint64_t getAttackers(const BitBoard& board, BoardPosition target, uint64_t occ)
    {
        ...
        /* NOTE: for pawns we use white attack mask with black pieces as we want the pieces that
         * attack the given square not the attack from the given square */
        return (s_pawnAttackMaskTable[PlayerBlack][target] & board.pieces[WhitePawn])
            | (s_pawnAttackMaskTable[PlayerWhite][target] & board.pieces[BlackPawn])
