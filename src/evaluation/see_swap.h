...
class SeeSwap {
public:
    static inline bool run(const BitBoard& board, movegen::Move move, int32_t margin)
    {
        if (move.isCastleMove()) {
            return margin >= 0;
        }

        int32_t score = -margin;
        ...
        if (move.takeEnPessant()) {
            ...
            score += s_pieceValues[Pawn];
        } else if (move.isCapture()) {
            const Piece target = board.getTargetAtSquare(toSquare, board.player).value();
            score += s_pieceValues[target];
        }

        if (promotionPiece.has_value()) {
            score -= s_pieceValues[Pawn];
            score += s_pieceValues[promotionToColorlessPiece(move.promotionType()).value()];
        }
        ...
        while (attackers) {
            ...
            if ((player == board.player && score >= 0) || (player != board.player && score <= 0)) {
                break;
            }
