...
public:
    static inline int32_t run(const BitBoard& board, movegen::Move move)
    {
        ...
        Piece nextPiece = promotionPiece.has_value()
            ...
            : board.getAttackerAtSquare(fromSquare, player).value();
        ...
        while (true) {
            ...
            /* only handle attackers that are still part of our custom occupation */
            attackers &= occ;
