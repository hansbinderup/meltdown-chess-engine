...
static inline TermScore getPassedPawnsScore(const BitBoard& board, TermContext& ctx)
{
    ...
    utils::bitIterate(passedPawns, [&](BoardPosition pos) {
        ...
        /* always reward having the possibility to be a passed pawn */
        ADD_SCORE_INDEXED(passedPawnBonus, row);

        /* if we can push the pawn without it being attacked -> free push */
        ...
        /* if we can push the pawn into being defended -> protected pawn */
