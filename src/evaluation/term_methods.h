...
struct TermContext {
    ...
    using AttackArray = std::array<uint64_t, magic_enum::enum_count<Piece>()>;
...
static inline TermScore getStaticKingPawnScore(const BitBoard& board, TermContext& ctx)
{
    ...
    const uint64_t ourPawns = board.pieces[Pawn] & board.occupation[player];
    const uint64_t theirPawns = board.pieces[Pawn] & board.occupation[opponent];

    const auto ourKingPos = utils::lsbToPosition(board.pieces[King] & board.occupation[player]);
    const auto theirKingPos = utils::lsbToPosition(board.pieces[King] & board.occupation[opponent]);
...
static inline TermScore getKnightScore(const BitBoard& board, TermContext& ctx, uint8_t& phaseScore)
{
    ...
    const uint64_t ourPawns = board.pieces[Pawn] & board.occupation[player];
    const uint64_t theirPawns = board.pieces[Pawn] & board.occupation[player];
...
static inline TermScore getRookScore(const BitBoard& board, TermContext& ctx, uint8_t& phaseScore)
{
    ...
    const uint64_t ourPawns = board.pieces[Pawn] & board.occupation[player];
    const uint64_t theirPawns = board.pieces[Pawn] & board.occupation[opponent];
    const uint64_t theirKings = board.pieces[King] & board.occupation[opponent];
...
static inline TermScore getPieceAttacksScore(const BitBoard& board, TermContext& ctx)
{
    ...
    for (const auto piece : { Pawn, Knight, Bishop, Rook, Queen }) {
        const int pawnCount = std::popcount(board.pieces[piece] & board.occupation[opponent] & ctx.pieceAttacks[player][Pawn]);
        const int knightCount = std::popcount(board.pieces[piece] & board.occupation[opponent] & ctx.pieceAttacks[player][Knight]);
        const int bishopCount = std::popcount(board.pieces[piece] & board.occupation[opponent] & ctx.pieceAttacks[player][Bishop]);
        const int rookCount = std::popcount(board.pieces[piece] & board.occupation[opponent] & ctx.pieceAttacks[player][Rook]);
        const int queenCount = std::popcount(board.pieces[piece] & board.occupation[opponent] & ctx.pieceAttacks[player][Queen]);

        ADD_SCORE_MULTI_INDEXED(pawnAttacks, pawnCount, piece);
        ADD_SCORE_MULTI_INDEXED(knightAttacks, knightCount, piece);
        ADD_SCORE_MULTI_INDEXED(bishopAttacks, bishopCount, piece);
        ADD_SCORE_MULTI_INDEXED(rookAttacks, rookCount, piece);
        ADD_SCORE_MULTI_INDEXED(queenAttacks, queenCount, piece);
    }
...
static inline TermScore getChecksScore(const BitBoard& board, TermContext& ctx)
{
    ...
    const uint64_t ourPawnsPushed = utils::pushForward<player>(board.pieces[Pawn] & board.occupation[player]);

    /* fetch king's position and compute each piece attack from there */
    const BoardPosition theirKingPos = utils::lsbToPosition(board.pieces[King] & board.occupation[opponent]);
...
static inline TermScore getPawnPushThreatScore(const BitBoard& board, TermContext& ctx)
{
    ...
    const uint64_t targets = board.occupation[opponent] & ~theirPawns;
    ...
    const uint64_t pushes = utils::pushForward<player>(ourPawns) & ~board.occupation[Both];
    ...
    utils::bitIterate(attacks, [&](BoardPosition pos) {
        ...
        ADD_SCORE_INDEXED(pawnPushThreats, *target);
    });
...
static inline TermScore getPassedPawnsScore(const BitBoard& board, TermContext& ctx)
{
    ...
    const uint64_t theirPawns = board.pieces[Pawn] & board.occupation[opponent];
    const uint64_t theirKing = board.pieces[King] & board.occupation[opponent];

    const uint64_t passedPawns = ctx.passedPawns[player];
    const auto theirKingPos = utils::lsbToPosition(theirKing);
    const bool kingPawnsOnly = board.occupation[opponent] == (theirPawns | theirKing);
