...
constexpr static inline void clearPiece(uint64_t& piece, BoardPosition pos, Piece type, BitBoard& newBoard)
{
    ...
    core::hashPiece(type, pos, newBoard.hash); // remove from hash
    core::hashPiece(type, pos, newBoard.materialHash); // remove from hash
}

constexpr static inline void setPiece(uint64_t& piece, BoardPosition pos, Piece type, BitBoard& newBoard)
{
    piece |= utils::positionToSquare(pos);
    core::hashPiece(type, pos, newBoard.hash); // add to hash
    core::hashPiece(type, pos, newBoard.materialHash); // remove from hash
}
...
{
    clearPiece(piece, fromPos, type, newBoard);
    setPiece(piece, toPos, type, newBoard);
}
...
constexpr void performCastleMove(BitBoard& newBoard, movegen::Move move)
{
    ...
    case CastleWhiteKingSide: {
        movePiece(newBoard.pieces[WhiteKing], fromPos, toPos, WhiteKing, newBoard);
        movePiece(newBoard.pieces[WhiteRook], H1, F1, WhiteRook, newBoard);
    } break;

    case CastleWhiteQueenSide: {
        movePiece(newBoard.pieces[WhiteKing], fromPos, toPos, WhiteKing, newBoard);
        movePiece(newBoard.pieces[WhiteRook], A1, D1, WhiteRook, newBoard);
    } break;
    case CastleBlackKingSide: {
        movePiece(newBoard.pieces[BlackKing], fromPos, toPos, BlackKing, newBoard);
        movePiece(newBoard.pieces[BlackRook], H8, F8, BlackRook, newBoard);
    } break;
    case CastleBlackQueenSide: {
        movePiece(newBoard.pieces[BlackKing], fromPos, toPos, BlackKing, newBoard);
        movePiece(newBoard.pieces[BlackRook], A8, D8, BlackRook, newBoard);
    } break;
    case CastleNone:
        assert(false);
        break;
...
constexpr void performPromotionMove(BitBoard& newBoard, movegen::Move move)
{
    ...
    clearPiece(pawns, move.fromPos(), type, newBoard);
    ...
    if (move.isCapture()) {
        if (const auto victim = newBoard.getTargetAtSquare<player>(move.toSquare())) {
            clearPiece(newBoard.pieces[victim.value()], move.toPos(), victim.value(), newBoard);
    ...
    case PromotionQueen: {
        ...
        setPiece(newBoard.pieces[type], move.toPos(), type, newBoard);
    } break;
...
constexpr void performEnpessantMove(BitBoard& newBoard, movegen::Move move)
{
    ...
    movePiece(newBoard.pieces[ourPawn], fromPos, toPos, ourPawn, newBoard);
    clearPiece(newBoard.pieces[theirPawn], capturePos, theirPawn, newBoard);
...
constexpr BitBoard performMove(const BitBoard& board, movegen::Move move)
{
    ...
    if (move.isCastleMove()) {
        ...
        movePiece(newBoard.pieces[pieceType], fromPos, toPos, pieceType, newBoard);
